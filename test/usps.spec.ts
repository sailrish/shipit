...
describe('usps client', () => {
  ...
  describe('generateRequest', () => {
    ...
    it(
      'includes TrackFieldRequest in the track request document',
    ...
    it(
      'includes user ID in the track field request',
      done => _xmlParser.parseString(_xmlDoc, function (err, doc) {
    ...
    it(
      'includes revision 1 in the track field request',
      done => _xmlParser.parseString(_xmlDoc, function (err, doc) {
    ...
    it(
      'includes client IP in the track field request',
      done => _xmlParser.parseString(_xmlDoc, function (err, doc) {
    ...
    it(
      'includes source ID in the track field request',
      done => _xmlParser.parseString(_xmlDoc, function (err, doc) {
  ...
  describe('integration tests', () => {
    ...
    describe('pre-shipment package', () => {
      beforeAll(
        done => fs.readFile('test/stub_data/usps_pre_shipment.xml', 'utf8', (err, xmlDoc) => _uspsClient.presentResponse(xmlDoc, 'trk', function (err, resp) {
      ...
      it(
        'has a service of Package Service',
        () => expect(_package.service).toBe('Package Services')
      );

      it(
        'has a destination of Kihei, HW',
    ...
    describe('delivered package', () => {
      beforeAll(
        done => fs.readFile('test/stub_data/usps_delivered.xml', 'utf8', (err, xmlDoc) => _uspsClient.presentResponse(xmlDoc, 'trk', function (err, resp) {
          expect(err).toBeFalsy();
          _package = resp;
          return done();
        }))
      ...
      it(
        'has a service of first class package',
        () => expect(_package.service).toBe('First-Class Package Service')
      );

      it(
        'has a destination of Chicago',
    ...
    describe('out-for-delivery package', () => {
      beforeAll(
        done => fs.readFile('test/stub_data/usps_out_for_delivery.xml', 'utf8', (err, xmlDoc) => _uspsClient.presentResponse(xmlDoc, 'trk', function (err, resp) {
          expect(err).toBeFalsy();
          _package = resp;
          return done();
        }))
