...
describe('fedex client', function() {
  ...
  return describe('integration tests', function() {
    ...
    describe('delivered package', function() {
      before(done => fs.readFileSync('test/stub_data/fedex_delivered.xml', 'utf8', (err, xmlDoc) => _fedexClient.presentResponse(xmlDoc, 'trk', function(err, resp) {
    ...
    return describe('in transit package with an activity with missing location', function() {
      before(done => fs.readFileSync('test/stub_data/fedex_missing_location.xml', 'utf8', (err, xmlDoc) => _fedexClient.presentResponse(xmlDoc, 'trk', function(err, resp) {
        should.not.exist(err);
        _package = resp;
        return done();
      })));
