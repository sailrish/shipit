  ...
  AmazonClient = class AmazonClient extends ShipperClient {
    ...
    getEta(data) {
      ...
      if (arrival != null ? arrival.match('today') : undefined) {
        ...
        for (let month of Array.from(MONTHS)) {
        ...
        if (foundMonth) {
          ...
          for (let day_of_week in DAYS_OF_WEEK) {
    ...
    getActivitiesAndStatus(data) {
      ...
      const {$, response} = data;
      for (let row of Array.from($('#tracking-events-container').children('.a-container').children('.a-row'))) {
